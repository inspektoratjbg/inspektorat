...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.surat_masuk.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form action="{{ route("admin.surat_masuk.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('lampiran') ? 'has-error' : '' }}">
                ...
                <input type="file" id="lampiran" name="lampiran[]" class="form-control" value="{{ old('lampiran', isset($surat_masuk) ? $surat_masuk->lampiran : '') }}" multiple>
            ...
            <div class="form-group {{ $errors->has('disposisi') ? 'has-error' : '' }}">
                ...
                <select name="disposisi[]" id="roles" class="form-control select2" multiple="multiple">
                    @foreach($disposisi as $id => $disposisi)
                        <option value="{{ $disposisi->disposisi }}" {{ (in_array($id, old('disposisi', [])) || isset($surat_masuk) && $surat_masuk->disposisi->contains($id)) ? 'selected' : '' }}>{{ $disposisi->disposisi }}</option>
                    @endforeach
                </select>
