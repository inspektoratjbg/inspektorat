...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.surattugas.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.surattugas.update", [$surattugas->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('lampiran') ? 'has-error' : '' }}">
                ...
                <input type="file" id="lampiran" name="lampiran[]" class="form-control" value="{{ old('lampiran', isset($surattugas) ? $surattugas->lampiran : '') }}" multiple>
